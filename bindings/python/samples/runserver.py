...
class Weather:
    ...
    def meanTemps(cls):
        ...
        if time.time() - cls.lastInsideUpdate > cls.insideTimeInterval:
            cls.updateInsideTemps()
    ...
    def outsideTemperature(cls):
        # On met a jour si necessaire
        if time.time() - cls.lastOutsideUpdate > cls.outsideTimeInterval:
            cls.updateOutsideTemps()
        return cls.out
...
class ServerHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        while True:
            ...
            # Command dispatch
            if command == "CLEAR":
                self.server.server_runner.clear()
            elif command == "HOUR":
                self.server.server_runner.hour()
                ...
                command == "GET"
                and len(commands) > 1
                and commands[1].startswith("/CLEAR")
            ):
                self.server.server_runner.reset()
                self.server.server_runner.timeBeforeIdle = time.time()
                ...
                command == "GET"
                and len(commands) > 1
                and commands[1].startswith("/HISTORY")
            ):
...
class RunServer(SampleBase):
    ...
    def run(self):
        ...
        Weather.openweathermap_apikey = (
            open(self.args.openweathermap_apikey).read().strip()
        )
