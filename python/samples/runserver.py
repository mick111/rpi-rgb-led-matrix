        ...
        while True:
            ...
            if timeBeforeDimming < 0 or (self.hour is None and self.text is None and self.images is None):
                ...
                graphics.DrawText(
                                  ...
                                  "{}{:02d}°".format(time.strftime("%H:%M" if int(timeBeforeDimming) % 2 == 0 else "%H %M"), outsideTemperature()) # Data to draw
                                  )
                graphics.DrawText(
                  self.offscreen_canvas, # Canvas destination
                  self.fontLittle,       # Font to show
                  0, 15,                  # Position
                  graphics.Color(255, 255, 255), # Color
                  u"{:02d}°{:02d}°".format(insideTemperature("salon"), insideTemperature("chambre")) # Data to draw
                )
